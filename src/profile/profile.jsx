...
export default function Profile() {
  ...
  const renderSidebarLink = (path, label, icon) => {
    ...
    return (
      <NavLink
        ...
        className={`w-full flex items-center gap-2 px-3 py-1.8 rounded-lg transition-all outline-none ${isActive
          ? 'bg-secondary text-white font-bold ring-2 ring-secondary/30 shadow-sm'
      ...
      >
        ...
        <span className="font-label-md text-label-md text-left text-xs">{label}</span>
      </NavLink>
    )
  }
  ...
  return (
    <div className="bg-background text-on-surface min-h-screen flex flex-col lg:flex-row relative">
      ...
      <aside className={`fixed lg:sticky top-0 inset-y-0 left-0 lg:inset-auto z-50 lg:z-auto w-[240px] bg-white shadow-2xl lg:shadow-none border-r border-outline-variant/30 flex flex-col p-3 space-y-4 transition-transform duration-300 ease-in-out lg:translate-x-0 lg:h-screen lg:flex shrink-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
        }`}>
        <div className="flex items-center justify-between px-1 py-2 mb-1">
          <div className="text-left">
            <a className="font-headline-xl text-headline-xl font-black text-secondary tracking-tighter cursor-pointer block text-left" href="#" onClick={(e) => { e.preventDefault(); navigate('/'); }}>SEAPEDIA</a>
        ...
        <nav className="flex-grow overflow-y-auto pr-1 flex flex-col justify-between h-full space-y-6">
            ...
            {user.activeRole === 'BUYER' && (
              <>
                <div className="space-y-1">
                  <p className="text-[9px] text-on-surface-variant font-bold uppercase tracking-wider px-3 mb-1">Utama</p>
                  {renderSidebarLink('/buyer/dashboard', 'Dashboard', 'dashboard')}
                  {renderSidebarLink('/buyer/notifications', 'Notifikasi', 'notifications')}
                </div>
                <div className="space-y-1">
                  <p className="text-[9px] text-on-surface-variant font-bold uppercase tracking-wider px-3 mb-1">Fitur &amp; Layanan</p>
                  {renderSidebarLink('/buyer/addresses', 'Daftar Alamat', 'map')}
                  {renderSidebarLink('/buyer/orders', 'Daftar Pesanan', 'shopping_bag')}
                </div>
                <div className="space-y-1">
                  <p className="text-[9px] text-on-surface-variant font-bold uppercase tracking-wider px-3 mb-1">Pengaturan</p>
          ...
          <div className="space-y-4 pt-2 border-t border-outline-variant/10">
            {user.activeRole !== 'ADMIN' && (
              <div className="pb-1 px-1 space-y-1.5">
                <p className="text-[9px] text-on-surface-variant font-bold uppercase tracking-wider">Ganti Peran</p>
                  ...
                  {['BUYER', 'SELLER', 'DRIVER'].filter(r => r !== user.activeRole).map((role) => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full flex items-center gap-2 px-2.5 py-1.5 text-[11px] text-secondary hover:bg-secondary/10 rounded-lg transition-colors font-bold text-left outline-none"
                      >
            ...
            <div className="space-y-1">
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-1.8 text-error hover:bg-red-50 rounded-lg transition-colors outline-none font-bold"
              >
                <span className="material-symbols-outlined text-[18px]">logout</span>
                <span className="font-label-md text-label-md text-left text-xs">Keluar</span>
              </button>
            </div>
          </div>
        </nav>
